<template>
  <ion-page>
    <ion-header :translucent="true">
      <ion-toolbar>
        <ion-title>Daftar Lengkap Mainan</ion-title>
      </ion-toolbar>
    </ion-header>
...
const handleSubmit = async (todo: Omit<Todo, 'id' | 'createdAt' | 'updatedAt' | 'status'>) => {
  ...
  try {
    if (editingId.value) {
      ...
      await showToast('Mainan Berhasil diubah', 'success', checkmarkCircle);
    } else {
      await firestoreService.addTodo(todo as Todo);
      await showToast('Mainan Berhasil Ditambahkan', 'success', checkmarkCircle);
    }
    loadTodos();
  } catch (error) {
...
const handleDelete = async (deleteTodo: Todo) => {
  try {
    ...
    await showToast('Daftar terpilih berhasil dihapus', 'success', checkmarkCircle);
    loadTodos();
  } catch (error) {
